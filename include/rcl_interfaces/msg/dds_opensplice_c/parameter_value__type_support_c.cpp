...
#include "rosidl_generator_c/primitives_array.h"  // bytes_value
#include "rosidl_generator_c/primitives_array_functions.h"  // bytes_value
#include "rosidl_generator_c/string.h"  // string_value
#include "rosidl_generator_c/string_functions.h"  // string_value

// forward declare type support functions

using __dds_msg_type = rcl_interfaces::msg::dds_::ParameterValue_;
using __ros_msg_type = rcl_interfaces__msg__ParameterValue;
...
convert_ros_to_dds(const void * untyped_ros_message, void * untyped_dds_message)
{
  ...
  // Field name: string_value
  {
    ...
    dds_message->string_value_ = DDS::string_dup(str->data);
  }
  ...
  // Field name: bytes_value
  {
    ...
    for (DDS::ULong i = 0; i < size; ++i) {
      auto & ros_i = ros_message->bytes_value.data[i];
      dds_message->bytes_value_[i] = ros_i;
    }
  }
...
publish(void * dds_data_writer, const void * ros_message)
{
  ...
  dds_message.string_value_._retn();
  switch (status) {
    case DDS::RETCODE_ERROR:
...
convert_dds_to_ros(const void * untyped_dds_message, void * untyped_ros_message)
{
  ...
  // Field name: bool_value
  {
    ros_message->bool_value = dds_message->bool_value_ == TRUE;
  }

  // Field name: integer_value
  {
    ros_message->integer_value = dds_message->integer_value_;
  }

  // Field name: double_value
  {
    ros_message->double_value = dds_message->double_value_;
  }
  ...
  // Field name: string_value
  {
    ...
    if (!succeeded) {
      return "failed to assign string into field 'string_value'";
    }
  }
  ...
  // Field name: bytes_value
  {
    ...
    if (ros_message->bytes_value.data) {
      rosidl_generator_c__byte__Array__fini(&ros_message->bytes_value);
    }
    if (!rosidl_generator_c__byte__Array__init(&ros_message->bytes_value, size)) {
      return "failed to create array for field 'bytes_value'";
    }
    for (DDS::ULong i = 0; i < size; i++) {
      auto & ros_i = ros_message->bytes_value.data[i];
      ros_i = dds_message->bytes_value_[i];
    }
  }
