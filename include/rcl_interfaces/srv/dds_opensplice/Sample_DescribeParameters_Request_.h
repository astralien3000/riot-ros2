//******************************************************************
// 
//  Generated by IDL to C++ Translator
//  
//  File name: Sample_DescribeParameters_Request_.h
//  Source: rcl_interfaces/srv/dds_opensplice/Sample_DescribeParameters_Request_.idl
//  Generated: timestamp removed to make the build reproducible
//  OpenSplice V6.4.140407OSS
//  
//******************************************************************
#ifndef _SAMPLE_DESCRIBEPARAMETERS_REQUEST__H_
#define _SAMPLE_DESCRIBEPARAMETERS_REQUEST__H_

#include "sacpp_mapping.h"
#include "sacpp_DDS_DCPS.h"
#include "DescribeParameters_Request_.h"


namespace rcl_interfaces
{
   namespace srv
   {
      namespace dds_
      {
         struct Sample_DescribeParameters_Request_;

         struct Sample_DescribeParameters_Request_
         {
               DDS::ULongLong client_guid_0_;
               DDS::ULongLong client_guid_1_;
               DDS::LongLong sequence_number_;
               DescribeParameters_Request_ request_;
         };

         typedef DDS_DCPSStruct_var < Sample_DescribeParameters_Request_> Sample_DescribeParameters_Request__var;
         typedef DDS_DCPSStruct_out < Sample_DescribeParameters_Request_> Sample_DescribeParameters_Request__out;
      }
   }
}




#endif 
